...
import {
  DumpTools
} from "./chunk-ZV7FGGD4.js";
import "./chunk-I7GPDIG2.js";
import {
  AreIndices32Bits,
...
var GLTFMaterialExporter = class {
  ...
  async _getImageDataAsync(buffer, width, height, mimeType) {
    ...
    engine.isWebGPU ? await import("./pass.fragment-SEWSX62O.js") : await import("./pass.fragment-5HHADSLF.js");
  ...
  _getPixelsFromTexture(babylonTexture) {
    const pixels = babylonTexture.textureType === Constants.TEXTURETYPE_UNSIGNED_BYTE ? babylonTexture.readPixels() : babylonTexture.readPixels();
    return pixels;
  }
...
var rotation180Y = new Quaternion(0, 1, 0, 0);
var epsilon2 = 1e-6;
...
function ConvertToRightHandedNode(value) {
  ...
  if (translation.equalsWithEpsilon(defaultTranslation, epsilon2)) {
...
function CollapseParentNode(node, parentNode) {
  ...
  if (parentTranslation.equalsWithEpsilon(defaultTranslation, epsilon2)) {
  ...
  if (parentScale.equalsWithEpsilon(defaultScale, epsilon2)) {
...
function BuildMorphTargetBuffers(morphTarget, mesh, bufferManager, bufferViews, accessors, convertToRightHanded) {
  ...
  if (morphTarget.hasPositions) {
    ...
    const originalPositions = mesh.getVerticesData(VertexBuffer.PositionKind, void 0, void 0, true);
  ...
  if (morphTarget.hasNormals) {
    ...
    const originalNormals = mesh.getVerticesData(VertexBuffer.NormalKind, void 0, void 0, true);
    if (originalNormals) {
  ...
  if (morphTarget.hasTangents) {
    ...
    const originalTangents = mesh.getVerticesData(VertexBuffer.TangentKind, void 0, void 0, true);
    if (originalTangents) {
  ...
  if (morphTarget.hasColors) {
    ...
    const originalColors = mesh.getVerticesData(VertexBuffer.ColorKind, void 0, void 0, true);
...
var GLTFExporter = class _GLTFExporter {
  ...
  _extensionsPostExportNodeAsync(context, node, babylonNode, nodeMap, convertToRightHanded) {
    return this._applyExtensions(node, (extension, node2) => extension.postExportNodeAsync && extension.postExportNodeAsync(context, node2, babylonNode, nodeMap, convertToRightHanded, this._bufferManager));
  }
  ...
  async generateGLBAsync(glTFPrefix) {
    ...
    headerBufferView.setUint32(0, 1179937895, true);
    headerBufferView.setUint32(4, 2, true);
    headerBufferView.setUint32(8, byteLength, true);
    ...
    jsonChunkBufferView.setUint32(4, 1313821514, true);
    ...
    binaryChunkBufferView.setUint32(4, 5130562, true);
  ...
  _setCameraTransformation(node, babylonCamera, convertToRightHanded, parent) {
    ...
    if (parent !== null) {
      ...
      cameraLocal.decompose(void 0, rotation, translation);
    } else {
      babylonCamera.getWorldMatrix().decompose(void 0, rotation, translation);
    }
  ...
  _exportBuffers(babylonRootNodes, state) {
    ...
    const morphTagetsMeshesMap = /* @__PURE__ */ new Map();
    for (const babylonNode of babylonRootNodes) {
    ...
    for (const buffer of buffers) {
      ...
      for (const vertexBuffer of vertexBuffers) {
        const { byteOffset, byteStride: byteStride2, type, normalized } = vertexBuffer;
        const size = vertexBuffer.getSize();
        const meshes = vertexBufferToMeshesMap.get(vertexBuffer);
        ...
        switch (vertexBuffer.getKind()) {
          case VertexBuffer.NormalKind:
          case VertexBuffer.TangentKind: {
            EnumerateFloatValues(bytes, byteOffset, byteStride2, size, type, maxTotalVertices * size, normalized, (values) => {
          ...
          switch (vertexBuffer.getKind()) {
            ...
            case VertexBuffer.TangentKind: {
              for (const mesh of vertexBufferToMeshesMap.get(vertexBuffer)) {
                const { byteOffset, byteStride: byteStride2, type, normalized } = vertexBuffer;
                const size = vertexBuffer.getSize();
                EnumerateFloatValues(bytes, byteOffset, byteStride2, size, type, mesh.getTotalVertices() * size, normalized, (values) => {
                  values[0] = -values[0];
                });
              }
            }
...
var NAME16 = "KHR_texture_transform";
...
var KHR_texture_transform = class {
  ...
  postExportTexture(context, textureInfo, babylonTexture) {
    ...
    if (babylonTexture.wAng !== 0) {
      if (babylonTexture.uRotationCenter !== 0 || babylonTexture.vRotationCenter !== 0) {
        if (babylonTexture.homogeneousRotationInUVTransform && babylonTexture.uScale !== babylonTexture.vScale) {
          Tools.Warn(`${context}: Texture ${babylonTexture.name} with homogenousRotationInUVTransform, non-uniform scaling, and non-zero rotation cannot be exported with ${NAME16}.`);
          return;
        }
        Tools.Warn(`${context}: Texture ${babylonTexture.name} with non-origin rotation center will be exported using an adjusted offset with ${NAME16}.`);
    ...
    textureInfo.extensions[NAME16] = textureTransform;
  }
};
GLTFExporter.RegisterExtension(NAME16, () => new KHR_texture_transform());
export {
